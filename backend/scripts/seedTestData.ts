 ...
 *
 * Creates a handful of accounts, gives everyone a spread of logs, reviews and
 * votes, and wires up friendships in every state the UI has to render:
 * accepted both ways, a request waiting on the main account, and a request the
 * main account has sent.
 *
 * Safe to re-run. Accounts are matched by email and reused, logs upsert on
 * (user_id, game_id), and friendships upsert on the ordered pair.
 *
 * Never point this at production: it writes directly with the service-role key
 * and creates confirmed auth users.
 */
...
const main = async () => {
    ...
    /* Popular games, so the covers and descriptions are recognisable when
                    ...
                    .upsert(
                        {
                            ...
                            game_id: game.id,
                            body: REVIEW_BODIES[
                                (personIndex + i) % REVIEW_BODIES.length
                            ]!,
                            is_public: true,
                        },
                        { onConflict: "user_id,game_id" },
    ...
    for (const [i, reviewId] of reviewIds.entries()) {
        // A varying number of voters per review, so "most helpful" has an
        // order rather than a wall of ones.
        ...
        const { error } = await db.from("friendships").upsert(
            {
                ...
                requested_by:
                    user.relation === "request-received" ? other : main.id,
            },
    ...
    /* The test accounts are friends with each other too, so a friend feed
       has something in it from more than one direction. */
