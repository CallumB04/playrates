...
export const createMyFriendsRouter = ({ service, requireAuth }: Deps): Router => {
    const router = Router();
    ...
    router.post(
        ...
        async (req, res) => {
            const { userId } = req.valid!.body as z.infer<
...
export const createUserFriendsRouter = ({
    ...
    router.get(
        ...
        async (req, res) => {
            ...
            const { status } = req.valid!.query as z.infer<
                typeof FriendQuerySchema
            >;
