...
export const createMyReviewsRouter = ({ service, requireAuth }: Deps): Router => {
    const router = Router();
...
export const createGameReviewsRouter = ({
    ...
    router.get(
        ...
        async (req, res) => {
            ...
            res.json(
                await service.listByGame(gameId, req.auth?.userId, pagination)
            );
        }
    );
