...
import { PROFILE_ACCENT_SLUGS } from "@playrates/shared";
...
/* The column is text with a CHECK behind it, but a row read back is still
   just a string. Anything that is not one of ours reads as no choice, which
   is the username's colour. */
const toAccent = (value: string | null): ProfileAccent | null =>
