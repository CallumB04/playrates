...
import { DEFAULT_ACCENT, PROFILE_ACCENT_SLUGS } from "@playrates/shared";
...
/* The column is text with a CHECK behind it, but a row read back is still
   just a string. Anything that is not one of ours is the brand, so a profile
   is never returned without a colour. */
